...
//            lcd_puts("Telemeter");
        ...
        case tele_action:
            enable_interrupts();
            enterLPM(lpm_mode);  // waiting to receive angle from PC
            tele_angle_int = atoi(angle_char_arr);
            move_servo(tele_angle_int);
            while(state_telemeter == tele_action){
                meas_send_angle_and_dis(tele_angle_int); // send measured distance and angle to pc
...
void servo_scan(int angle1, int angle2, int dist_or_light_flag){
    ...
    TIMER_A1_config();
    TIMER_A0_config_for_script();
    curr_angle = (angle1 * 10) + 100;     // angle*(2000-200)/180 + 100     // innitialize
    last_angle =      (angle2 * 10) + 100;     // angle*(2000-200)/180 + 100     // up to 180 deg
...
int measure_distance(){
    ...
    pwmOutTrigConfig();
    TIMER_A0_config_for_ultrasonic();
    //enable_timerA1();
    enterLPM(lpm_mode);

    if (diff > 0 && diff < 65535){
        distance = (diff/58); // convert from smclk cycles to distance
    }
...
//------------------------------------------------------------------------------
// Move the ultrasonic sensor to degree p and show the degree and distance onto PC screen
//------------------------------------------------------------------------------

void meas_send_angle_and_dis(int p){
    char lcd_print_str[4] = {'0','0','0','\0'};
    meas_and_send_distance(); // send distance to PC
    __delay_cycles(250000);
    calc_and_send_angle(p);   // send angle to PC

    lcd_init();
    lcd_puts("Measuring Distance... ");
    lcd_cursor2();
...
void meas_and_send_distance(){
    int distance;
    int i;
    distance = measure_distance(); // measure distance using ultrasonic sensor

     // convert distance(int) to array of chars
     for (i = 2; i >= 0; --i, distance /= 10)
     {
         dist_char_arr[i] = (distance % 10) + 48; // convert int to char
     }
...
void sample_LDR_x(int LDR_index){
    ADCsampLDRconfig();
    int sample = 0;
    ...
    for (i = 3; i >= 0; --i, sample /= 10)
        {
        LDR_val[i] = (sample % 10) + '0';
        }
}
    ...
    while (sleep_state_flag == 0){
     ...
     // after receiving a whole line:
        ...
        switch(opcode_int){
            ...
            case 6:
                move_servo(arg1_int);
                meas_send_angle_and_dis(arg1_int);
