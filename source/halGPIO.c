...
char angle_char_arr [4] = {'0','0','0','\n'};
int tele_index = 0;
int tele_angle_int = 0;
int tele_flag = 0;
int tele_str_ind = 0;
int dist_char_arr[4] = {'0','0','0','\n'};
...
//---------------------------------------------------------------------
void send_distance_to_pc(){
    tele_flag = 1;
    tele_str_ind = 0;
    IE2 |= UCA0TXIE;
    UCA0TXBUF = dist_char_arr[tele_str_ind];
}

//---------------------------------------------------------------------
//           Send telemeter angle string
//---------------------------------------------------------------------
void send_angle_to_pc(){
    tele_flag = 2;
    tele_str_ind = 0;
    IE2 |= UCA0TXIE;
    UCA0TXBUF = angle_char_arr[tele_str_ind];
}
...
#endif
{
        ...
        if(tele_flag == 1){
            UCA0TXBUF = dist_char_arr[tele_str_ind++];
            if (dist_char_arr[tele_str_ind-1] == '\n'){           // check if TX over
                IE2 &= ~UCA0TXIE;
                tele_flag = 0;
            }
        }
        // send angle to PC
            ...
            UCA0TXBUF = angle_char_arr[tele_str_ind++];
            if (angle_char_arr[tele_str_ind-1] == '\n'){           // check if TX over
                IE2 &= ~UCA0TXIE;
                tele_flag = 0;
            }
        }
