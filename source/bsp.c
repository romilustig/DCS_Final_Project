...
void TIMER_A1_config(void){     /* First timer (A1) for sampling the LDRs using the ADC module,
                              * for PWMOUT (Servo) using outmod7  */
    ISR_FLAG = 0;
    Timer_1_CTL =  MC_1 +Timer_1_SSEL2 ;             //Continious up CCR0 + SMCLK
}

void TIMER_A0_config_for_script(void){    /* Second Timer (A0) for performing script commands with 500ms interval default (will be changed to D)  */
...
void TIMER_A0_config_for_ultrasonic(void){    /* third Timer config for ultrasonic sensor */
